...
CDN="https://cdn.coollabs.io/zpanel"
DATE=$(date +"%Y%m%d-%H%M%S")

OS_TYPE=$(grep -w "ID" /etc/os-release | cut -d "=" -f 2 | tr -d '"')
ENV_FILE="/data/zpanel/source/.env"
DOCKER_VERSION="27.0"
# TODO: Ask for a user
CURRENT_USER=$USER
...
echo -e "Welcome to Zpanel Installer!"
echo -e "This script will install everything for you. Sit back and relax."
echo -e "Source code: https://github.com/freqkflag/Zpanel-dev/blob/main/scripts/install.sh"
...
if [ -f "/data/zpanel/source/.env" ] && [ "$DOCKER_POOL_BASE_PROVIDED" = false ] && [ "$DOCKER_POOL_SIZE_PROVIDED" = false ]; then
    ENV_DOCKER_ADDRESS_POOL_BASE=$(grep -E "^DOCKER_ADDRESS_POOL_BASE=" /data/zpanel/source/.env | cut -d '=' -f2 || true)
    ENV_DOCKER_ADDRESS_POOL_SIZE=$(grep -E "^DOCKER_ADDRESS_POOL_SIZE=" /data/zpanel/source/.env | cut -d '=' -f2 || true)
...
mkdir -p /data/zpanel/{source,ssh,applications,databases,backups,services,proxy,webhooks-during-maintenance,sentinel}
mkdir -p /data/zpanel/ssh/{keys,mux}
mkdir -p /data/zpanel/proxy/dynamic

chown -R 9999:root /data/zpanel
chmod -R 700 /data/zpanel

INSTALLATION_LOG_WITH_DATE="/data/zpanel/source/installation-${DATE}.log"
...
curl -fsSL $CDN/docker-compose.yml -o /data/zpanel/source/docker-compose.yml
curl -fsSL $CDN/docker-compose.prod.yml -o /data/zpanel/source/docker-compose.prod.yml
curl -fsSL $CDN/.env.production -o /data/zpanel/source/.env.production
curl -fsSL $CDN/upgrade.sh -o /data/zpanel/source/upgrade.sh
...
if [ -f "$ENV_FILE" ]; then
    ...
    awk -F '=' '!seen[$1]++' "$ENV_FILE" "/data/zpanel/source/.env.production" > "$ENV_FILE.tmp" && mv "$ENV_FILE.tmp" "$ENV_FILE"
    ...
    cp "/data/zpanel/source/.env.production" "$ENV_FILE"
fi
...
set +e
IS_ZPANEL_VOLUME_EXISTS=$(docker volume ls | grep zpanel-db | wc -l)
set -e

if [ "$IS_ZPANEL_VOLUME_EXISTS" -eq 0 ]; then
    echo " - Generating SSH key."
    test -f /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal && rm -f /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal
    test -f /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub && rm -f /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
    ssh-keygen -t ed25519 -a 100 -f /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal -q -N "" -C zpanel
    chown 9999 /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal
    sed -i "/zpanel/d" ~/.ssh/authorized_keys
    cat /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub >>~/.ssh/authorized_keys
    rm -f /data/zpanel/ssh/keys/id.$CURRENT_USER@host.docker.internal.pub
fi
...
if [[ $- == *x* ]]; then
    bash -x /data/zpanel/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
else
    bash /data/zpanel/source/upgrade.sh "${LATEST_VERSION:-latest}" "${LATEST_HELPER_VERSION:-latest}" "${REGISTRY_URL:-ghcr.io}" "true"
fi
...
echo -e "\nWARNING: It is highly recommended to backup your Environment variables file (/data/zpanel/source/.env) to a safe location, outside of this server (e.g. into a Password Manager).\n"
